#!/usr/bin/env python3
"""
Preprocessing script for Amazon Health and Household dataset.
Generates aws-health-time.txt with temporal and popularity features.

Input:  raw/aws-health/Health_and_Household.jsonl
Output: prep/aws-health/aws-health-time.txt

Usage:
    python prep_aws_health.py --user_core 30  # User 30-core, no item filtering
    python prep_aws_health.py --user_core 50  # User 50-core, no item filtering
...
def filter_user_core(data, min_user=30):
...
def main():
    parser = argparse.ArgumentParser(description='Preprocess AWS Health dataset')
    parser.add_argument('--user_core', type=int, default=30,
                        help='Minimum interactions per user (default: 30)')
    args = parser.parse_args()

    raw_path = os.path.join('raw', 'aws-health', 'Health_and_Household.jsonl')
    output_dir = os.path.join('prep', 'aws-health')
    output_path = os.path.join(output_dir, 'aws-health-time.txt')
